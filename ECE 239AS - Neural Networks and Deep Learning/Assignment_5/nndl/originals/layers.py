...
def affine_backward(dout, cache):
  ...
  dw = np.dot(np.transpose(x_r),dout)
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    sample_var = np.sum(np.power((x-sample_mean),2),axis=0)/N
    running_mean = momentum*running_mean + (1-momentum)*sample_mean
    running_var = momentum*running_var + (1-momentum)*sample_var
    ...
    cache = (x, x_hat, beta, gamma, sample_mean, sample_var, bn_param)
    ...
    # Normalize the activations with the batch mean and variance.
...
def batchnorm_backward(dout, cache):
  ...
  x, x_hat, beta, gamma, mu, sigma2, bn_param  = cache
  eps = bn_param.get('eps', 1e-5)
  N,_ = x.shape
  ...
  db = (x-mu)*dxhat
  dc = -(1/se)*db
  de = (1/2)*np.sqrt(1/se)*dc
  dsigma2 = np.sum(de,axis=0)
  dx = da + (2*(x-mu)/N)*dsigma2 + (1/N)*dmu
...
def dropout_forward(x, dropout_param):
  ...
  if mode == 'train':
    ...
    mask = (1/p)*(np.random.random_sample(x.shape) <= p)
