...
def rmsprop(w, dw, config=None):
  ...
  eav = config['learning_rate']/(np.sqrt(config['a'])+config['epsilon'])
  next_w = w - np.multiply(eav,dw)
...
def adam(w, dw, config=None):
  ...
  a = beta2*config['a'] + (1-beta2)*np.multiply(dw,dw)
 
  learning_rate = config['learning_rate']
  epsilon = config['epsilon']
  
  v_t = v/(1-np.power(beta1,t))
  a_t = a/(1-np.power(beta2,t))
  a_t = np.absolute(a_t)
  eav = learning_rate/(np.sqrt(a_t)+epsilon)
  next_w = w - np.multiply(eav,v_t)
